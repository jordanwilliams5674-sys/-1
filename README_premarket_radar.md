...
- 系统现在会把夜盘单独识别为 `夜盘`，不再混成“非交易时段”。
- 现有公开免费源对美股夜盘实时成交支持不稳定；如果没有夜盘专用字段，系统会回退到最后可得盘后价，并明确标注这只是线索，不当作充分量价确认。
- 如果后续要做真正的夜盘实时报价，需要接入专门的 overnight venue 数据源，而不是继续只依赖 Yahoo / TradingView 的常规延长时段字段。
...
- `current_actual_holdings`：旧兼容字段；当前保持为空，原实仓标的已转入观察池。
